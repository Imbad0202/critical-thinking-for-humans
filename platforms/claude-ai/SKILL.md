...
1. **Session tally** — events buffer in session context and are folded into a running tally at checkpoints (end of an item; end of a scene — a frame or fallacy round's commitment and process record flush together at scene end; a configure round flushes without a commitment carrier — its process record and every companion fold (`configure` line counts, `recent_misses`, `discards`) commit at the same checkpoint — and only after its challenge window closes — modes/scene.md). It lives and dies with this conversation.
